'''
Precompute CLAHE-processed versions of the images needed, saving to
data/isic2017/Image_clahe/ (same ID, same .npy format as Image/, so the
existing SkinDataset_csv loading code works unchanged aside from swapping
the image subdirectory). Labels are untouched -- CLAHE only modifies the
RGB image.

By default processes the union of image IDs found across a list of meta CSVs
(so re-running with a bigger ID list only computes the delta). Defaults cover
the FULL k0 split -- train, held-out val, AND the fixed test set -- since the
DullRazor precompute run initially missed the held-out val slice and nearly
crashed training at first validation.
'''
import os
import sys
import numpy as np
...
if __name__ == '__main__':
    csvs = sys.argv[1:] if len(sys.argv) > 1 else [
        ...
        'test_meta_kfold_meta_isic2017_train2000_0.csv',
