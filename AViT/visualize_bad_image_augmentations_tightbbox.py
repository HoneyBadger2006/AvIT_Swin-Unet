"""
Re-verification set for the bad-image crop augmentations AFTER the bbox fix
(margin removed -- tight GT bbox only). Covers all 33 bad images (CLAHE-only,
in-sample, fold-0), no sampling.

For each bad image, saves one full-resolution comparison PNG with 3 panels:
  1. Original + GT overlay + the computed tight bounding box drawn in yellow
...
OUT_DIR = '../per_image_analysis_v2/bad_image_augmentation/visual_verification_tightbbox'
CONTACT_SHEET_DIR = '../per_image_analysis_v2/bad_image_augmentation/contact_sheets_tightbbox'
IMAGES_PER_PAGE = 8
...
def load_all(img_id):
    ...
    return orig_img, orig_lbl, cropA_img, cropA_lbl, cropB_img, cropB_lbl


def main():
    bad_df = pd.read_csv(BAD_IMAGES_CSV, dtype={'image_id': str})
    bbox_df = pd.read_csv(BBOX_CSV, dtype={'image_id': str}).set_index('image_id')
    assert len(bad_df) == 33, 'expected 33 bad images, got {}'.format(len(bad_df))
    ...
    for _, row in bad_df.iterrows():
        ...
        orig_img, orig_lbl, cropA_img, cropA_lbl, cropB_img, cropB_lbl = load_all(img_id)

        fig, axes = plt.subplots(1, 3, figsize=(15, 5.5))
        fig.suptitle('{}  (fold-0 in-sample dice={:.3f})  tight bbox=({},{})-({},{})'.format(
            img_id, dice, *bbox), fontsize=12)

        overlay_mask(axes[0], orig_img, orig_lbl, 'Original + GT + tight bbox', bbox=bbox)
    ...
    for page in range(n_pages):
        ...
        for i, (_, row) in enumerate(chunk.iterrows()):
            ...
            overlay_mask(axes[i, 0], orig_img, orig_lbl, '{} (dice={:.3f}) Original+GT+tight bbox'.format(img_id, dice), bbox=bbox)
