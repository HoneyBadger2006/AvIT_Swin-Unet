...
# logits internally (mode='binary' -> focal_loss_with_logits), so it must be called
# with the pre-sigmoid model output, not the sigmoid probability.
focal_loss_fn = FocalLoss(mode='binary', alpha=0.25, gamma=2.0)


def compute_loss(config, criterion, output, label):
    """Returns (loss, sigmoid_output). All models use the original unweighted
    BCELoss + dice_loss on sigmoid-activated output, except SwinUnet.
    SwinUnet paper uses 0.4*CrossEntropyLoss + 0.6*DiceLoss, but that CE term assumes
    multi-channel softmax logits + integer class-index targets (Synapse, 9 classes).
    Our output is a single-channel sigmoid map, so CrossEntropyLoss doesn't apply as-is;
    BCEWithLogitsLoss (binary cross-entropy on raw logits) is the binary equivalent, and
    is used here in its place, weighted the same as the paper (0.4 CE-analog / 0.6 dice).
    ...
    if config.train.use_focal_loss:
        loss = loss + config.train.focal_lambda * focal_loss_fn(output, label)
...
def main(config):
    ...
    if config.model == 'SwinSeg':
        ...
        swin_ckpt_path = '{}/pretrained/swin_tiny_patch4_window7_224.pth'.format(config.pretrained_folder)
        if os.path.exists(swin_ckpt_path):
            model.load_from(DotDict({'MODEL': {'PRETRAIN_CKPT': swin_ckpt_path}}))
        else:
            print('WARNING: {} not found, SwinUnet will train from random init.'.format(swin_ckpt_path))
...
if __name__=='__main__':
    ...
    parser.add_argument('--test_model_dir', type=str, default=None,
                        help='Checkpoint path to load when --only_test is set.')
    parser.add_argument('--meta_csv_name', type=str, default=None,
                        help='Override which meta CSV under the dataset folder is used as the '
                             'fold-rotation source (defaults to meta_<dataset>.csv).')
    parser.add_argument('--fixed_test_csv_name', type=str, default=None,
                        help='If set, evaluate every fold against this fixed meta CSV (identical '
                             'across all folds) instead of the fold\'s own held-out slice.')
