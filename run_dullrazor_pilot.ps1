...
$driverLog = "$logDir\driver_dullrazor_pilot.log"
...
# DullRazor pilot: SwinUnet k0 + AViT k0, baseline loss (no focal), on the
# DullRazor-preprocessed images (Image_dullrazor), same official train2000/test600
# split used for the existing baseline (0.8275 / 0.7594) so results are comparable.
$runs = @(
  @{exp="isic2017_swinunet_dullrazor_k0"; model="SwinUnet"; k=0},
  @{exp="isic2017_avit_dullrazor_k0"; model="SwinSeg_CNNprompt_adapt"; k=0}
)
